...
- 静默降级可见（`src/trace-logger.ts` + `src/trader.ts` + `src/risk.ts` + `src/report-store.ts`）：TraceLogger 加 `warnings[]` 收集器 + `recordWarning()`；trader/risk 的 P0 兜底位点插桩——position_pct 同义词回退、numeric 字段落 0、TRADER_PLAN 缺失回退、风控 status 默认 pass（error 级）、有硬约束却未提取到仓位 cap。warnings 落 summary JSON + `run_summary.json`，dashboard 列表加 ⚠ 计数徽章、详情页顶部红框告警条。此前 position_pct=0 这类 bug 让"报告看着正常"的盲区无法 review（曾导致 600600 cap 绑定静默失效）；插桩由 `run*` 函数负责而非污染纯解析函数签名，位点从 18 压到 ~8
- 全链路跨阶段一致性检查（`src/cross-stage-checks.ts` + `src/orchestrator.ts`）：新增 6 条确定性检查（+0 LLM 成本），在 `runFullAnalysis` 末尾、所有阶段产出后运行——目标价离谱/方向与现价矛盾、止损价在错侧、保守风控 reject 却被 pass、分析师 consensus 与研究方向背离、retries 耗尽伪 pass、reject 仍出执行计划（error 级）。结果落 summary JSON 的 `cross_stage_issues`，dashboard 详情页顶部与 warnings 合并成 Review 告警条。现有质量门只覆盖分析师阶段（数据引用），下游跨阶段结构异常此前无人拦截
