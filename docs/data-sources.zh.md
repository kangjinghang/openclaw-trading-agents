...
| trading-news | 个股新闻 + 宏观新闻 | 东方财富 / akshare（全球快讯） | — | `requests`, `akshare` |
| trading-sentiment | 市场情绪 / 涨停情绪池 | 东方财富 | akshare (zt_pool) | `akshare` |
| trading-policy | 政策事件 | 东方财富搜索 / akshare（全球快讯） | — | `requests` |
| trading-hot-money | 北向资金/主力资金/板块资金流/龙虎榜 | 同花顺（北向/资金流/板块）/东方财富（龙虎榜） | akshare | `akshare`, `py-mini-racer`, `requests` |
...
- 个股新闻：东方财富搜索 API
- 宏观/全球新闻：东方财富全球快讯（akshare `stock_info_global_em`）
...
- 政策事件：东方财富搜索 API
- 宏观快讯：东方财富全球快讯（akshare `stock_info_global_em`）

### 资金流向（`trading-hot-money`）

- 北向资金（沪股通/深股通）：同花顺 `data.hexin.cn` 直连 API（无需反爬 token）
- 个股主力资金流向（主力/散户净流入）：同花顺 `stock_fund_flow_individual`（akshare，需 py_mini_racer 算 hexin-v token）。全局批量预取一次（~5191 只股），每股 O(1) 查表注入，替代旧的东财 push2his 单股路（被封禁，已删除）
- 板块资金流排名：行业板块主力净流入 inflow/outflow top8（同花顺 `stock_fund_flow_industry`，90 个行业）。旧源东财 `push2` clist 已废弃
- 龙虎榜：东方财富数据中心（`datacenter-web.eastmoney.com`，非被封的 `push2`），含席位明细

> **行业体系归一化**（重要）：`fundamentals.industry`（标的行业，来自东财数据中心 `BOARD_NAME`）与 sector 榜单（同花顺 hyzjl）分类体系相近（均参考申万细分），但命名有差异——东财 `BOARD_NAME` 带申万罗马数字后缀（如「白酒Ⅱ」「保险Ⅱ」「银行Ⅱ」），同花顺板块名不带（「白酒」「保险」「银行」）。若直接 `板块名.includes(行业)` 匹配，"白酒".includes("白酒Ⅱ")=false，会导致白酒/银行等大板块股误判为"未上榜"。
>
> **解法**（`data-fetcher.ts` parseHotMoney 内）：匹配前对两端行业名归一化——去末尾申万后缀（`[ⅠⅡⅢⅣⅤ二三四五]+$`），再做双向包含（`a===b || a.includes(b) || b.includes(a)`），覆盖两边命名粒度差异。匹配结果驱动板块轮动标签（主线/弱势/未上榜）注入 shallow-analyzer prompt。
...
| `fundamentals` | `fundamentals/tencent`、`fundamentals/mootdx`、`fundamentals/em_datacenter`、`fundamentals/em_quarterly`、`fundamentals/em_consensus`、`fundamentals/akshare` | 多源拼装；`em_datacenter` 独占行业/公司名 |
| `news` | `news/stock_em`、`news/macro_akshare` | 宏观：东方财富全球快讯（akshare）单源 |
| `policy` | `policy/stock_em`、`policy/macro_akshare` | 同 news |
| `sentiment` | `sentiment/hot_rank`、`sentiment/zt_pool` | 均东方财富 |
| `hot_money` | `hot_money/northbound`、`hot_money/fund_flow`、`hot_money/hot_stocks`、`hot_money/dragon_tiger`、`hot_money/sector_fund_flow` | fund_flow/sector_fund_flow 同花顺（需 py_mini_racer）；northbound 同花顺直连；dragon_tiger 东财数据中心 |
...
| `push2.eastmoney.com` IP 限流（Connection aborted） | ~~`hot_money/fund_flow`、`hot_money/sector_fund_flow`~~（已切同花顺）；`trading-sector` | 历史：fund_flow/sector_fund_flow 间歇性失败；现已解决 | fund_flow/sector_fund_flow 改用同花顺（需 `py-mini-racer==0.6.0`，详见 AGENTS.md）；`trading-sector` 仍用 push2，强制 IPv4 + ≥1s 节流 |
| `py_mini_racer` 在 Windows 死锁（mini-racer 新包冲突） | `hot_money/fund_flow`、`hot_money/sector_fund_flow` | `MiniRacer()` 初始化无限卡死，rebalance 被 exec-python 30s timeout 杀进程 | 装**老包** `py-mini-racer==0.6.0`（同步实现，Win 稳定），**不装**新包 `mini-racer`（后台 event loop 线程 Win 死锁）。两包都写 `py_mini_racer/` 目录，新包盖老包导致 import 到死锁代码。详见 AGENTS.md Environment |
| `push2.eastmoney.com` IPv6 TLS 重置 | `trading-sector`（独立 skill，未走 health 追踪） | 行业排名可能返回空 | 同上：强制 IPv4 |
| 百度股市通 `getrelatedblock` API 返回 403 | `trading-sector`（独立 skill） | 概念板块返回 `null` | 暂无备源；数据省略 |
