...
| `push2.eastmoney.com` IP 限流（Connection aborted） | `hot_money/fund_flow`、`hot_money/sector_fund_flow` | 间歇性失败；同 IP 持续 ~15min+ | `http_helpers.py` 强制 IPv4 + ≥1s 节流；fundamentals 行业已改用 `datacenter-web.eastmoney.com`（不受 push2 限流）；hot_money script 内 try/except 优雅降级 |
