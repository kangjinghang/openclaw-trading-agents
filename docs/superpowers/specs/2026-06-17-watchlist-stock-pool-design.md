...
> 日期：2026-06-17
> 状态：已实现（第 0 层数据源偏离设计，见第 8 节"实现偏离"）
...
### 实现偏离：第 0 层数据源改用 akshare

设计指定东财 clist，但**实现改用 akshare** `stock_info_a_code_name`。原因：实测东财 clist 频繁被限流，稳定性不足。影响：

- 覆盖：akshare 约 5207 只（排除北交所），比东财 5533 只少约 326 只
