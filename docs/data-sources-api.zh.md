...
- [trading-fundamentals（估值 + 财务）](#trading-fundamentals估值--财务) — 6 子源：腾讯估值/mootdx 财报/em 基础信息/em 季度趋势/em 机构预期/akshare 三大报表
- [trading-news（个股新闻 + 宏观）](#trading-news个股新闻--宏观) — 2 子源：东方财富个股搜索/akshare 宏观（东方财富全球快讯）
- [trading-sentiment（情绪 + 涨停池）](#trading-sentiment情绪--涨停池) — 2 子源：东方财富热门排行/akshare 涨停池
- [trading-policy（政策事件）](#trading-policy政策事件) — 2 子源：东方财富个股搜索/akshare 宏观（东方财富全球快讯）
...
| `vol` | `volume` | `float(row.get('vol', 0)) * 100` | **× 100（手 → 股）**，避免直接传手数导致 LLM 误用 100x 过小值 |
...
kline.py 不只返回 raw K 线，还计算两段 markdown 文本注入到 output 顶层字段，避免 LLM 自己算涨跌幅/技术指标出错。
...
1. **mootdx/akshare `vol` 单位是"手"**：脚本已 `× 100` 转"股"，跨源一致
2. **akshare `日期` 无时间部分**："2026-06-12" vs mootdx "2026-06-12 15:00"，跨源对齐注意（VPA 表格只取日期部分）
...
>
> 健康追踪子源：`fundamentals/tencent`、`fundamentals/mootdx`、`fundamentals/em_datacenter`、`fundamentals/em_quarterly`、`fundamentals/em_consensus`、`fundamentals/akshare`
...
> 注：实时 TDX 协议（tdxpy `get_finance_info`）固定 32 字段，**不含**资产负债率/毛利率——这两个比率只存在于 mootdx `Affair` 历史财报 zip。等价数据已由其他子源提供：资产负债率见 `financial_health.debt_ratio_pct`，毛利率见 `quarterly_trends.gross_margin`（东财 datacenter，真实可用）。
...
### 子源 3: `fundamentals/em_datacenter`（股票基本信息，唯一源）

**协议**: HTTP（`datacenter-web.eastmoney.com`，**不受 push2 IP 限流影响**）
...
### 子源 4: `fundamentals/em_quarterly`（季度财务趋势）
...
### 子源 5: `fundamentals/em_consensus`（机构一致预期）
...
fundamentals 的 6 个子源是**并行采集 + 各自独立容错**，无 fallback 链：
...
# 行业/公司信息：em_datacenter 唯一源（无 push2 兜底）
try:
    data["stock_info"] = ...       # datacenter
except Exception as e:
    data["stock_info_error"] = str(e)
```

每个子源失败只影响自身字段，下游（parseFundamentals）对缺失字段降级为 0/空。
...
```json
{
  ...
  "_calls": [
    ...
    {"stage": "fundamentals/em_datacenter", "success": true, "error": null, "duration_ms": 150},
...
1. **datacenter `sortColumns` 大小写敏感**：`REPORTDATE` 不是 `REPORT_DATE`，错写返回 null 无报错
2. **datacenter `"result": null` 显式 null**：`d.get("result", {})` 在 null 时返回 None，须用 `(d.get("result") or {})` 防御
3. **东财 IPv6 连接重置**：`http_helpers.py` 全局强制 IPv4（影响所有东财请求）
4. **PEG 仅正增长时计算**：`eps_growth_pct <= 0` 时 `peg` 字段不存在
5. **`forward_pe` 依赖两个子源**：需 `valuation.price` + `consensus_eps_next` 都成功
6. **akshare 财报 NaN 值**：sina 缺失值用 NaN，`math.isfinite()` 过滤为 None 避免非法 JSON
7. **akshare `扣非净利润` 不可得**：sina 利润表不暴露，`financial_health.notes` 标注此限制
8. **OCF/净利/资本开支为累计值**：季报跨期比较需注意 `period_type`（Q1/H1/Q3/FY）
9. **`financial_health` success 以"有数据"为准**：akshare 缺失/拉取失败/无重叠报告期 → `_fetch_financial_health` 返回 None，`record_call` 报 `success=False`（不再误报成功）

## trading-news（个股新闻 + 宏观）

> 文件：`skills/trading-news/scripts/news.py`（277 行）
>
> 健康追踪子源：`news/stock_em`、`news/macro_akshare`
...
### 子源 2: `news/macro_akshare`（宏观新闻，唯一源）
...
宏观新闻单源（eastmoney），无 fallback：

```python
data["macro_news"] = _fetch_global_news_akshare() or []
data["macro_news_source"] = "eastmoney"  # 固定值（移除 cls 后单源）
```
...
```json
{
  ...
  "data": {
    ...
    "macro_news": [
      {"title": "央行降准0.5个百分点", "content": "...", "time": "2026-06-14 09:15", "source": "东方财富"}
    ],
    "macro_news_source": "eastmoney"
  },
  "source": "eastmoney",
  "_calls": [
...
1. **`macro_news_source` 固定为 `"eastmoney"`**：宏观新闻单源（akshare `stock_info_global_em`），akshare 拉取失败时 `macro_news` 为空数组
2. **JSONP 解析依赖 `(` `)` 字符**：东方财富改版可能破坏，异常被捕获返回空列表
...
>
> 健康追踪子源：`policy/stock_em`、`policy/macro_akshare`
...
### 子源 2: `policy/macro_akshare`（宏观政策，唯一源）
...
宏观政策单源（eastmoney），无 fallback，同 news 宏观：
```python
data["macro_policy_news"] = _fetch_macro_policy_akshare() or []
data["macro_policy_source"] = "eastmoney"  # 固定值
```
...
```json
{
  ...
  "data": {
    ...
    "macro_policy_news": [
      {"date": "2026-06-14", "title": "国务院常务会议部署稳增长措施", "content": "...", "source": "东方财富"}
    ],
    "macro_policy_source": "eastmoney"
  },
  "source": "eastmoney",
  "_calls": [
    {"stage": "policy/stock_em", "success": true, "error": null, "duration_ms": 800},
    {"stage": "policy/macro_akshare", "success": true, "error": null, "duration_ms": 1500}
  ]
}
```
...
1. **与 news 脚本高度同构**：宏观新闻逻辑相同，差异在 limit、字段名（`date` vs `time`）、lookback_days（30 vs 7）
2. **东方财富搜索基于关键词**：用股票代码搜索，返回不一定全是政策新闻，LLM 需自行判断相关性
3. **`lookback_days` 影响个股政策范围**：宏观新闻取最新 N 条不做日期过滤
4. **articles 列表必须在 try 外初始化**：否则异常路径会触发 `UnboundLocalError`
...
3. **push2 IPv6 连接重置**：`http_helpers.py` 全局强制 IPv4
...
**协议**: HTTP（东方财富公告 API `np-anotice-stock.eastmoney.com`）
...
**公告分类与重要性评分**（`_classify_announcement`）:
...
1. **ann API 过滤"解禁"公告**：lockup_history/upcoming 已覆盖解禁数据，避免重复计算
...
- 转换逻辑用代码块
- 失败示例必须给真实 JSON
- 已知陷阱要给具体场景（如"vol 单位是手不是股"、"日期字段无时间部分"），让读者一眼看懂
