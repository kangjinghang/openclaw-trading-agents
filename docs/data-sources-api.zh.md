...
4. **push2 IP 限流 per-subdomain**：push2 限流时 datacenter 正常，datacenter 是可靠兜底
...
1. **CLS telegraphList 接口不稳定**：曾连续多天 ConnectionError，旧代码 `bare except: pass` 吞错，现 `raise` + `macro_news_error` 字段 + akshare 兜底
...
2. **CLS 失效是已知问题**：与 news 共享同一问题，`macro_policy_source` 追踪实际源
3. **东方财富搜索基于关键词**：用股票代码搜索，返回不一定全是政策新闻，LLM 需自行判断相关性
4. **`lookback_days` 影响个股政策范围**：宏观新闻取最新 N 条不做日期过滤
5. **articles 列表必须在 try 外初始化**：否则异常路径会触发 `UnboundLocalError`
