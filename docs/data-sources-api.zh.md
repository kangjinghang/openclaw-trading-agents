...
| 失败模式 | 触发条件 | 行为 |
|---|---|---|
| 腾讯接口失败 | 网络/超时 | `valuation_error` 记录原因，其余子源继续 |
| mootdx 未安装 | `ImportError` | `financial_snapshot_error` 记录，其余继续 |
| mootdx F10 返回空 | `fin is None or fin.empty` | 跳过快照，不报错 |
| push2 限流 | IP 被 ban（`ConnectionError` 持续） | `stock_info_push2_error` 记录 → 自动 fallback datacenter |
| datacenter 也失败 | 罕见（不同子域名） | `stock_info_error = "both push2 and datacenter returned no data"` |
| 季度财务无数据 | 新股/退市股 | `quarterly_trends_error` 记录 |
| 机构预期无覆盖 | 小盘股/新股 | `consensus_eps` 返回 `None`，不设 `consensus_eps_error` |
| akshare 未安装 | `ImportError` | `financial_health_error` 记录 |
| 三大报表无交集 | 新股报表不全 | `_derive_financial_health` 返回 `None` |

### 已知陷阱

1. **datacenter `sortColumns` 字段名大小写敏感**
   - `RPT_LICO_FN_CPD` 的日期列是 `REPORTDATE`（无下划线），不是 `REPORT_DATE`
   - 错误的 `sortColumns` 不会报错，但返回 `success=false, result=null`（commit `12f135e` 修复）
   - `RPT_WEB_RESPREDICT` 根本没有日期列，**不要加 sortColumns**

2. **datacenter `"result": null` 显式 null**
   - 不同于键缺失，`d.get("result", {})` 在 null 时返回 None 而非 {}
   - 必须用 `(d.get("result") or {}).get("data", [])` 防御（commit `12f135e`）

3. **push2 IPv6 连接重置**
   - 东财 push2 的 IPv6 地址经常连接重置，`http_helpers.py` 全局强制 IPv4（commit `6fdfc01`）

4. **push2 IP 限流是 per-subdomain**
   - `push2.eastmoney.com` 被限流时，`datacenter-web.eastmoney.com` 正常
   - 因此 datacenter 是 push2 的可靠兜底（commit `d0a36f7`）

5. **PEG 仅正增长时计算**
   - 当 `eps_growth_pct <= 0` 时，`peg` 字段不存在（不写入负值或 0）

6. **`forward_pe` 依赖两个子源都成功**
   - 需要 `valuation.price`（tencent）+ `consensus_eps_next`（em_consensus）
   - 任一失败则 `forward_pe` 不存在

7. **akshare 财报 NaN 值**
   - sina 返回缺失值用 NaN，`_derive_financial_health` 用 `math.isfinite()` 过滤为 `None`
   - 不处理会导致 `json.dumps` 输出 bare `NaN`（非法 JSON）

8. **akshare `扣非净利润` 不可得**
   - sina 利润表不暴露扣非净利润（仅财报附注中有）
   - `financial_health.notes` 显式标注此限制

9. **OCF/净利/资本开支为累计值**
   - 季报（Q1/H1/Q3）是累计值，跨期比较需注意 `period_type`
...
| 失败模式 | 触发条件 | 行为 |
|---|---|---|
| 东方财富搜索失败 | 网络/超时/JSONP 解析错误 | `stock_news` 返回 `[]`，`stock_news_error` 记录原因 |
| CLS 接口失效 | 网络/反爬/IP 封禁 | `raise RuntimeError` → fallback akshare |
| akshare 未安装 | `ImportError` | `raise RuntimeError` → `macro_news` 为空 |
| akshare 返回空 | 上游无数据 | `macro_news` 为空，`macro_news_error` 记录 |
| 所有宏观源失败 | CLS + akshare 都失败 | `macro_news_source = "none"`，错误链拼接 |

### 已知陷阱

1. **CLS telegraphList 接口不稳定**（commit `10017ee`）
   - 曾出现连续多天返回 ConnectionError 或空数据
   - 旧代码用 `bare except: pass` 静默吞掉错误 → `macro_news` 空了几天无人知晓
   - 修复后 `raise RuntimeError` + `macro_news_error` 字段 + akshare 兜底

2. **`macro_news_source` 字段是可观测性关键**
   - 值为 `"cls"` / `"akshare"` / `"none"`
   - 当值为 `"none"` 时说明两个宏观源都挂了，需要人工排查

3. **JSONP 解析依赖 `(` 和 `)` 字符**
   - 如果东方财富改版不加 JSONP wrapper，`text.index("(")` 会抛 `ValueError`
   - 异常被外层 `except` 捕获，返回空列表

4. **`content` 截取 300 字符**
   - 东方财富和 CLS 的 content 都做了 `[:300]` 截取
   - 下游 LLM 看到的是截断版本，不是全文

5. **时间分层 cutoff 基于 `date` 参数而非 `now()`**
   - `date` 是分析日期（用户传入），`now` = `date 23:59:59`
   - 周末/假日运行时，realtime_6h 可能为空（没有最新新闻）

6. **lookback_days 影响 history 层范围**
   - 默认 7 天，policy 角色传 14 天
   - realtime/extended 层不受影响（固定 6h/24h）
...
| 失败模式 | 触发条件 | 行为 |
|---|---|---|
| hot_rank push2 限流 | IP 被 ban | `hot_rank = None`，其余子源继续 |
| zt_pool 日期格式错误 | 非 `YYYY-MM-DD` | `record_call` 记录失败，返回 `None` |
| zt_pool 5 天内无交易日 | 长假/数据缺失 | `zt_pool = None` |
| akshare 未安装 | `ImportError` | zt_pool 回溯循环中 `except Exception: continue`，最终 `zt_pool = None` |
| 个股新闻失败 | 搜索 API 超时 | `stock_news = []`，`news_count = 0` |
| 连板数 NaN | 数据缺失 | `s == s` 过滤（NaN != NaN 技巧） |

### 已知陷阱

1. **push2 IP 限流影响 hot_rank**（commit MEMORY 中的 push2 限流条目）
   - `push2.eastmoney.com/api/qt/clist/get` 在高频探测后会 IP-ban ~15min+
   - hot_rank 失败时返回 `None`（不阻断其他子源），但情绪分析会缺市场宽度数据

2. **zt_pool 用的是 push2ex 不是 push2**
   - `push2ex.eastmoney.com` 与 `push2.eastmoney.com` 是不同子域名
   - push2 限流时 push2ex 仍可访问（per-subdomain 限流）

3. **akshare 列名版本漂移**
   - 不同 akshare 版本的 `stock_zt_pool_em` 列名可能微调（如 "连板数" vs "连板数_"）
   - 用 `_col(keyword)` 模糊匹配而非硬编码列名

4. **非交易日回溯最多 5 天**
   - 长假（如春节 7 天）可能超出 5 天窗口 → zt_pool 返回 `None`
   - `actual_date` 字段记录实际数据日期（可能 ≠ 入参 `date`）

5. **NaN 连板数过滤**
   - `raw_streaks` 中可能有 `None` 或 `NaN`
   - 用 `s is not None and s == s` 过滤（`NaN != NaN` 是 Python/IEEE 754 特性）

6. **`stock_news` 内部复用东方财富搜索 API**
   - 与 `news/stock_em` 是同一接口，但 sentiment 脚本独立调用
   - 两个脚本各自有独立的 `_fetch_stock_news_eastmoney`

7. **情绪评分是简单关键词匹配**
   - 不含 NLP/ML 模型，仅基于词表 hit count
   - 一条新闻同时命中正负面词时，按 hit 数多少判定正/负/中性
...
| 失败模式 | 触发条件 | 行为 |
|---|---|---|
| 东方财富搜索失败 | 网络/超时 | `stock_policy_news` 返回 `[]` |
| CLS 接口失效 | 网络/反爬 | `raise RuntimeError` → fallback akshare |
| akshare 未安装 | `ImportError` | `raise RuntimeError` → `macro_policy_news` 为空 |
| 所有宏观源失败 | CLS + akshare 都失败 | `macro_policy_source = "none"` |

### 已知陷阱

1. **与 news 脚本高度同构**
   - policy.py 和 news.py 的宏观新闻 fallback 逻辑几乎一模一样
   - 差异仅在 limit（20 vs 10）、字段名（`date` vs `time`）、lookback_days（30 vs 7）

2. **CLS 失效是已知问题**（commit `10017ee`）
   - 与 news 共享同一问题，同一批修复
   - `macro_policy_source` 字段用于追踪实际使用的源

3. **东方财富搜索是基于关键词的**
   - 用股票代码作为 `keyword` 搜索，返回的不一定是政策新闻
   - 可能包含公司公告、行业分析等——LLM 需要自行判断相关性

4. **`lookback_days` 同时用于 stock_policy 和（间接）影响宏观新闻范围**
   - stock_policy_news 按日期过滤（`>= cutoff`）
   - 宏观新闻不做日期过滤（取最新 N 条）

5. **`_fetch_policy_eastmoney` 的 articles 列表初始化**（commit `fa6dda9`）
   - 早期版本 `articles` 变量未在 try 块外初始化，异常路径导致 `UnboundLocalError`
   - 修复后在函数顶部 `articles = []`
...
| 失败模式 | 触发条件 | 行为 |
|---|---|---|
| 北向资金失败 | 同花顺接口超时 | `northbound = None` |
| 北向资金无数据 | `times` 数组为空 | `northbound = None` |
| push2 fund_flow 限流 | IP 被 ban | `fund_flow = None` |
| push2 sector_fund_flow 限流 | IP 被 ban（与 fund_flow 同子域名） | `sector_fund_flow = None` |
| 10jqka 接口错误 | `errocode != 0` | `hot_stocks = None` |
| 龙虎榜无数据 | 股票未上榜 | `dragon_tiger = []`（空列表，非 None） |

### 已知陷阱

1. **push2 IP 限流影响 fund_flow + sector_fund_flow（commit MEMORY 中 push2 限流条目）**
   - `push2.eastmoney.com/api/qt/clist/get`（sector_fund_flow）和 `push2/api/qt/stock/fflow/kline/get`（fund_flow）**共用同一子域名**
   - 两者会**同时**被限流（per-subdomain 限流特性）
   - 限流持续 ~15min+，期间北向资金（hexin）、龙虎榜（datacenter）、热门股（10jqka）不受影响

2. **`fund_flow = null` / `sector_fund_flow = null` 的语义**
   - `null` 表示**抓取失败**（push2 限流或网络错误），不是"没有资金流数据"
   - 下游 LLM 应将 null 解释为"数据不可用"，而非"主力净流入为零"

3. **push2 IPv6 连接重置**（commit `6fdfc01`）
   - `http_helpers.py` 全局强制 IPv4，所有 push2 调用受益

4. **北向资金单位是元不是亿元**
   - `hgt_close` / `sgt_close` 的原始值单位是**元**
   - 与 sector_fund_flow 的 `main_net_yi`（亿元）不同

5. **龙虎榜金额从元转万元**（`/ 10000`）
   - `BILLBOARD_NET_AMT` 等字段原始单位是**元**
   - output 的 `net_buy` / `buy_amt` / `sell_amt` 单位是**万元**

6. **板块资金流从元转亿元**（`/ 1e8`）
   - `f62` / `f136` 原始单位是**元**
   - output 的 `main_net_yi` / `super_net_yi` 单位是**亿元**

7. **hot_stocks 用 HTTP 不是 HTTPS**
   - URL 是 `http://zx.10jqka.com.cn/`（非加密）
   - 可能被某些网络环境拦截

8. **dragon_tiger 返回空列表而非 None**
   - 股票未上榜时返回 `[]`（而非 `null`）
   - 这与 fund_flow（返回 `None`）语义不同——空列表是"正常无数据"，None 是"抓取失败"

9. **em_get 的限流间隔不跨子源**
   - `em_get` 的 `_EM_MIN_INTERVAL`（1s + jitter）是全局的
   - 但 fund_flow 和 sector_fund_flow 都走 push2，IP-ban 后间隔无意义
...
| 失败模式 | 触发条件 | 行为 |
|---|---|---|
| datacenter 解禁查询失败 | 网络/接口异常 | `lockup_history` / `lockup_upcoming` 返回 `[]` |
| mootdx F10 失败 | mootdx 未安装/TCP 超时 | `insider_transactions` 返回 `[]` |
| ann API success=false | 接口返回非成功 | `announcements` 返回 `[]` |
| ann API 网络失败 | 网络/超时 | `announcements` 返回 `[]` |
| reduce 查询失败 | datacenter 异常 | `reduce_holdings` 返回 `[]` |
| 无未来解禁 | lockup_upcoming 为空 | `pressure_rating = "无明显压力"` |

### 已知陷阱

1. **ann API 过滤"解禁"类型公告**（commit `82b0e07`）
   - `_classify_announcement` 返回 `("解禁", 0)` 的公告被 `continue` 跳过
   - 因为 lockup_history / lockup_upcoming 已经覆盖了解禁数据
   - 避免重复计算解禁压力

2. **ann API 的 lookback 默认 60 天**
   - 比其他脚本（news 7 天、policy 30 天）更长
   - 因为公告事件（重组、增发等）的影响周期较长

3. **ann API 的 `success` 字段检查**
   - payload 必须有 `success: true`，否则视为失败
   - 与 datacenter API 的 `result != null` 检查不同

4. **insider_transactions 字段全部 str 化**
   - mootdx F10 返回的 DataFrame 字段类型不确定
   - 统一 `{k: str(v)}` 避免类型错误，但下游 LLM 需要自行解析数字

5. **lockup_upcoming 的 90 天窗口**
   - 固定 90 天前瞻（`forward_days=90`），不可配置
   - 超过 90 天的解禁不会出现在 upcoming 中

6. **reduce_em 用 `datetime.now()` 而非入参 `date`**
   - `REDUCE_DATE >= today` 而非 `REDUCE_DATE >= analysis_date`
   - 理论上应该是入参日期，但实际用了当前日期

7. **pressure_rating 仅基于条数，不含市值**
   - 3 条小比例解禁 = "重大压力"，1 条大比例解禁 = "中等压力"
   - 不够精确，但作为快速信号足够
