...
第2层 diff/{data_date}.json               今日 vs baseline 的新增异动（可重跑）
   ↓
第3层 derived/{data_date}-candidates.json        区间异动榜（持续上涨趋势，可重跑）
      derived/{data_date}-daily-candidates.json  单日异动榜（今日脉冲，可重跑）
```
...
| `src/watchlist/candidates.ts` | 第3层：区间榜 `buildCandidates` + 单日榜 `buildDailyCandidates` |
| `src/watchlist/ranker.ts` | 第4层：LLM 精排（LONG/SHORT 双组 + 强制分布 + 反模糊词） |
| `src/watchlist/rebalancer.ts` 等 | 第5层：组合调仓方案（见 [`rebalancer-pipeline.zh.md`](rebalancer-pipeline.zh.md)） |
| `src/diff-cli.ts` / `candidates-cli.ts` / `rank-cli.ts` / `rebalance-cli.ts` / `scan-all-cli.ts` | 单层 / 串跑 CLI |

> 已知问题：raw 膨胀（32MB/天 ≈ 8GB/年），保留策略（留 N 天 + gzip）待未来。详见交易日处理 spec §12.1。

### 第4-5 层（LLM 决策）

第3层后还有两层 LLM 决策（独立于单股 `trading_quick`/`trading_full`）：

- **第4层 ranker**（[`superpowers/specs/2026-06-18-llm-ranking-design.md`](superpowers/specs/2026-06-18-llm-ranking-design.md)）：178 候选 → top-15，分类排名（LONG/SHORT 分开），强制分布防 LLM 老好人，反模糊词规则
- **第5层 rebalancer**（[`rebalancer-pipeline.zh.md`](rebalancer-pipeline.zh.md)）：ranker top-N + 用户持仓 → 调仓方案（BUY/SELL/ADD/REDUCE/HOLD），11 条硬约束（单仓/行业/换手/现金/anti-churn/fitness 门槛）+ revise loop + execution-planner
