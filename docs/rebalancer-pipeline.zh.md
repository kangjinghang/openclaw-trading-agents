...
2. **没有**：技术指标 MACD/RSI（kline.py 有但 shallow 不注入）、解禁信息、历史分位
3. **没有**：成交量柱状图、K 线形态的详细分析（risk-role 另有 VPA 量价背离文本）
...
| fundamentals.py | `skills/trading-fundamentals/fundamentals.py` | `valuation.pe_ttm/pb` + `financial_snapshot.revenue/net_profit` + `stock_info.industry` + `quarterly_trends` + `consensus_eps` |
