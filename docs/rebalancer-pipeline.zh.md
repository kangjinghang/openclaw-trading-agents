...
| `news.py` | 个股新闻标题 | **主源**: 东方财富搜索 API（`searchapi.eastmoney.com`） | `news[].title` 最近 5 条标题 | 最近 7 天关于这只股的新闻 |
| `hot_money.py` | 资金流向 | **主源**: 东方财富 push2 API（`push2.eastmoney.com`） | `net_5d` 5 日净流入（万元） | 最近 5 天主力资金是流入还是流出 |
| `fundamentals.py` | 估值 + 财务 | **主源**: 东方财富 push2（PE/PB）+ datacenter（营收/净利）<br>**行业**: 东方财富 f127（`BOARD_NAME`） | `pe_ttm`, `pb`, `revenue_q1`, `net_profit_q1`, `stock_info.industry` | 市盈率、市净率、一季度营收和净利润、所属行业 |

**超时**: 每个脚本 30 秒硬上限。失败的脚本返回零值（PE=0, net_5d=0 等），不阻塞其他脚本。
...
Python 脚本原始输出
    ...
    │
    ├─ news → parseNews()
    │   └─ news: 最近 5 条新闻标题（字符串数组）
    │
    ├─ hot_money → parseHotMoney()
    │   └─ net_5d: 5 日净流入（万元，正=流入，负=流出）
    │
...
| 分数 | 含义 | 典型特征 |
|------|------|---------|
| 9-10 | 顶级 | 业绩已兑现 + 订单/产能可见 + 行业景气 + 估值合理 |
| 8    | 好   | 驱动明确 + 数据支撑 + 风险可控，但有一项未完全验证 |
| 7    | 还行 | 有逻辑但部分未验证，或估值偏高 |
| 5-6  | 弱   | 概念早期/传闻未证实/数据缺失 |
| ≤4   | 差   | 零营收/财务造假/退市风险 |
...
# 输出格式（严格 JSON）
{ "thesis": "...", "fitness_score": 0-10, "key_signals": [...], "data_gaps": [...] }
```

**关键观察**：
1. LLM 看到的**只有 4 类数据**：K 线统计量、新闻标题、资金流向、估值财务
2. **没有**：技术指标（MACD/RSI）、机构持仓、解禁信息、行业对比、历史分位
3. **没有**：成交量柱状图、K 线形态、支撑/阻力位的详细分析
4. LLM 的判断**高度依赖新闻标题的语义理解**——如果新闻标题模糊或缺失，fitness 会偏低
...
| **没有历史分位** | 不知道当前 PE 在历史上的百分位 | 无法判断"现在是贵还是便宜" |
