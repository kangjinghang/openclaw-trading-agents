...
exports.parsePositionPctSource = parsePositionPctSource;
exports.parsePositionPct = parsePositionPct;
exports.parseTraderPlan = parseTraderPlan;
exports.runTrader = runTrader;
 ...
 */
function buildRiskJudgeText(j) {
    const lines = [
        "## 风控反馈（上一轮计划被要求修订，必须严格遵守以下约束）",
        "",
        `**结论**：${j.verdict}${j.reason ? ` — ${j.reason}` : ""}`,
    ];
    const section = (title, items, must) => {
        if (items.length === 0)
            return;
        lines.push("");
 ...
 */
function parseNumericField(content, fieldPattern, isPercent = false) {
    const escaped = fieldPattern.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const suffix = '[^：:|\\n]*?';
    // Pattern 1: inline with colon — capture the full value line
    const re1 = new RegExp(`\\*{0,2}${escaped}${suffix}\\*{0,2}\\s*[：:]\\s*([^\\n]+)`);
    // Pattern 2: table cell
    const re2 = new RegExp(`\\|\\s*\\*{0,2}${escaped}[^|]*?\\*{0,2}\\s*\\|\\s*([^|]+)`);
 ...
 */
function parsePositionPctSource(content) {
    let v = parseNumericField(content, "建议仓位", true);
    if (v > 0)
        return { value: v, source: "建议仓位" };
    for (const label of ["减仓总量", "减仓比例", "总仓位", "建仓总量"]) {
        v = parseNumericField(content, label, true);
        if (v > 0)
            return { value: v, source: label };
    }
    return { value: 0, source: "none" };
}
function parsePositionPct(content) {
    return parsePositionPctSource(content).value;
}
function parseListSection(content, header) {
    ...
    const regex = new RegExp(`### \\d*[\\.、]?\\s*${header}[^\\n]*\\n([\\s\\S]*?)(?=\\n###|\\n<!-- |$)`);
 ...
 */
function parseTraderPlan(content) {
...
async function runTrader(researchDecision, analystReports, qualitySummary, config, openaiClient, traceLogger, ticker, date, riskJudge) {
    ...
    const userMessage = (0, prompt_loader_1.loadAndRender)("debate/trader.md", {
        ticker: ticker || "",
        date: date || "",
    ...
    const mappedDirection = direction === "Overweight"
        ? "Buy"
