...
function periodLabel(periodStr) {
    ...
    const unitWord = { d: "day", w: "week", m: "month", y: "year" };
    const plural = n === 1 ? "" : "s";
    return `last ${n} ${unitWord[match[2]]}${plural}`;
}
function applyPeriodToSources(state, since) {
    const out = {};
    for (const [name, entry] of Object.entries(state.sources)) {
        ...
        const filtered = (0, source_health_store_1.filterHistorySince)(entry.history, since);
        out[name] = {
            history: entry.history, // preserve raw history for --json consumers
            stats: (0, source_health_store_1.computeStats)(filtered),
            periodEmpty: filtered.length === 0,
        };
    }
    return out;
}
function renderTable(state, reportDir, periodStr, since) {
...
function renderFailing(state, since) {
    const filtered = applyPeriodToSources(state, since);
    // `--failing` skips sources with 0 calls in the period (they're not
    // "recently failing", just absent).
    const failing = Object.entries(filtered).filter(([_, e]) => !e.periodEmpty && e.stats.success_rate < 1);
    if (failing.length === 0) {
...
function extractPeriodArg(args) {
...
function main() {
    ...
    const periodArgPresent = args.some((a) => a.startsWith("--period=")) || args.includes("--period");
    ...
    if (periodArgPresent && periodRaw === undefined) {
        console.error(`error: --period requires a value. Valid: ${VALID_PERIODS.join(" | ")}`);
        process.exit(1);
    }
    const sinceParsed = (0, source_health_store_1.parsePeriod)(periodRaw);
    // User explicitly passed `--period <X>` but it failed to parse → reject loudly.
    if (periodArgPresent && sinceParsed === undefined) {
        console.error(`error: invalid --period value '${periodRaw}'. Valid: ${VALID_PERIODS.join(" | ")}`);
        process.exit(1);
    }
...
`);
    ...
    if (asJson) {
        ...
        const out = {
            ...state,
            sources: Object.fromEntries(Object.entries(filtered).map(([name, e]) => [
                name,
