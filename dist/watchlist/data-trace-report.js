...
// ── 6 个数据源的链路追踪 ────────────────────────────────────────────────────
function traceKline(d, idPrefix = "") {
...
function traceHotMoney(d, idPrefix = "") {
...
function traceFundamentals(d, idPrefix = "") {
...
function traceVpaMacd(d, idPrefix = "") {
...
function traceLockup(d, idPrefix = "") {
...
function renderStockPanel(ticker, stockData, stockReport, action, positionTrace) {
    ...
    h += details("展开 prompt 全文", codeBlockStyled((0, shallow_analyzer_1.formatAnalystPrompt)(stockData), "prompt"));
    if (stockReport) {
        // LLM 返回
        ...
        h += details("展开 prompt 全文", codeBlockStyled((0, shallow_analyzer_1.formatRiskPrompt)(stockData, mockAnalyst), "prompt"));
        // 决策链
...
function generateDataTraceReport(...entries) {
    if (entries.length === 0)
