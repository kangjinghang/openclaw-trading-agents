...
class PositionSimulator {
    ...
    async toHoldings(date, priceMap) {
    ...
    async applyPlan(result, date, reportsByTicker, priceMap) {
    ...
    async recordNav(date, actions, prevNav, priceMap) {
    ...
    /** 回测期末：把仍持有的仓位记为未平仓交易。 */
    async closeOpenPositions(endDate, priceMap) {
        for (const [ticker, pos] of this.positions) {
    ...
    getResults() {
        ...
        const avg = (arr) => arr.length > 0 ? arr.reduce((s, x) => s + x, 0) / arr.length : 0;
...
//# sourceMappingURL=backtest-simulator.js.map