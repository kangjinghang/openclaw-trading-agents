...
async function rebalancePipeline(input) {
    ...
    const entryPriceByPos = new Map();
    for (const p of input.holdings.positions)
        entryPriceByPos.set(p.ticker, p.entry_price);
    const held = new Map();
    ...
    const entryPriceByTicker = new Map();
    for (const p of input.holdings.positions)
        entryPriceByTicker.set(p.ticker, p.entry_price);
    const daysHeldByTicker = new Map();
    for (const [ticker, h] of held)
        daysHeldByTicker.set(ticker, h.days_held);
    const positionCtx = {
        reportsByTicker,
        volatilityByTicker,
