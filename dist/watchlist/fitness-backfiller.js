...
exports.backfillReturns = backfillReturns;
 ...
 */
async function backfillReturns(store, currentDate) {
    ...
    for (const rec of open) {
        ...
        const daysSince = Math.round((currentMs - decisionMs) / 86400000);
 ...
 */
async function computeReturns(rec) {
    const result = {};
    let raw;
    try {
        const res = await (0, exec_python_1.execSkillScript)("trading-kline", "kline", PROJECT_ROOT, [rec.ticker, "--count", "60"]);
        if (!res?.success)
            return result;
        raw = res.data;
    }
    ...
    const bars = Array.isArray(raw?.data) ? raw.data : [];
    if (bars.length === 0)
        return result;
    const findCloseOnOrAfter = (targetDate) => {
    ...
    if (entry === undefined || entry <= 0)
        return result; // 基准价拉不到，无法算收益
    const settle = (days, key) => {
...
//# sourceMappingURL=fitness-backfiller.js.map