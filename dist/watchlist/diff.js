...
function computeDiff(today, baseline) {
    ...
    for (const [ticker, todayEntry] of Object.entries(today.stocks)) {
        ...
        if (baselineEntry?.reason_list) {
            for (const r of baselineEntry.reason_list)
                baselineTs.add(r.timestamp);
        }
        const newReasons = (todayEntry.reason_list ?? []).filter((r) => !baselineTs.has(r.timestamp));
        const baselineRangeKeys = new Set();
        if (baselineEntry?.range_reason_list) {
            for (const r of baselineEntry.range_reason_list)
                baselineRangeKeys.add(rangeKey(r));
        }
        const newRanges = (todayEntry.range_reason_list ?? []).filter((r) => !baselineRangeKeys.has(rangeKey(r)));
