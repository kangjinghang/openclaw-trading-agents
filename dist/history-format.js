...
exports.formatHistoryCards = formatHistoryCards;
...
function normalizeDirection(raw) {
    ...
    if (!n)
        return null;
    if (["buy", "overweight", "看多", "多", "买入", "增持"].includes(n))
        return "Buy";
    if (["sell", "underweight", "看空", "空", "卖出", "减持"].includes(n))
        return "Sell";
    if (["hold", "neutral", "中性", "观望", "持有"].includes(n))
        return "Hold";
    return null;
}
/**
 ...
 */
function filterReports(reports, q) {
    const dirProvided = q.direction !== undefined && q.direction !== "";
    const wantDir = normalizeDirection(q.direction);
    return reports.filter((r) => {
        if (q.ticker && r.ticker !== q.ticker)
            return false;
        if (q.mode && r.mode !== q.mode)
            return false;
        if (q.date_from && r.date < q.date_from)
            return false;
        if (q.date_to && r.date > q.date_to)
            return false;
        if (dirProvided) {
            if (wantDir === null)
                return false; // 非法方向 → 不匹配任何
            if (normalizeDirection(r.direction) !== wantDir)
                return false;
        }
        return true;
    });
 ...
 */
function formatHistoryCards(filtered, shown, q) {
    if (shown.length === 0) {
    ...
    for (const r of shown) {
        ...
        const dur = (0, orchestrator_1.formatElapsed)(r.duration_ms || 0);
...
//# sourceMappingURL=history-format.js.map