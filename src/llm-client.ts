 ...
 */
export function parseResearchVerdict(
  content: string
): { direction: string; reason: string; bull_score?: number; bear_score?: number; confidence?: number } | null {
  ...
  const jsonStr = extractTaggedJson(content, "VERDICT");
  if (!jsonStr) return base;  // parsed via label/keyword fallback → no numeric block
...
function classifyDirection(text: string): string | null {
  ...
  if (buyScore === maxScore && buyScore > sellScore) return "Buy";
  if (sellScore === maxScore && sellScore > buyScore) return "Sell";
  return "Hold";
}
