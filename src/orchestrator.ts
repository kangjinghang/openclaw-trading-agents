...
import { runRiskDebate, runRiskManager, resolveMinStopLoss } from "./risk";
      ...
      try {
        ...
        if (verdict && verdict.direction !== "中性") {
          ...
          health.check("analyst_output", "warn", "suspicious_reason",
            ...
            { role: cfg.role, direction: verdict.direction, reason: verdict.reason }
          );
        }
...
export async function runFullAnalysis(
  ...
  // 对称，消除分散的正则逻辑。
  const { floor: minStopLoss, mismatch: stopLossMismatch } = resolveMinStopLoss(riskAssessment.judge);
