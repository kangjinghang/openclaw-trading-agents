...
import { AnalystReport, QualityGrade, QualitySummary, ScriptResult } from "./types";
...
export function checkNullFieldSentinels(
  ...
  for (const c of checks) {
    if (data[c.field] !== null) continue;  // only strict null = fetch failure
    const covered = sentinelTexts.some((t) => c.keywords.some((k) => t.includes(k)));
    if (!covered) missing.push(c.field);
  }
 ...
 */
export function checkDragonTigerContinuity(
  content: string,
  role: string,
  rawData: unknown,
): string | null {
  if (role !== "hot_money" || !rawData || typeof rawData !== "object") return null;
  const data = rawData as Record<string, unknown>;
  ...
  while ((m = regex.exec(content)) !== null) {
    ...
    if (n && n >= 2) claims.push(n);
  }
  if (claims.length === 0) return null;

  const maxClaim = Math.max(...claims);
  if (dt.length >= maxClaim) return null;
 ...
 */
function hardCheckReport(report: AnalystReport, rawData?: unknown): QualityGrade {
  const issues: string[] = [];
 ...
 */
export function validateAnalystReports(
  reports: AnalystReport[],
  dataResults?: Array<{ role: string; result: ScriptResult }>,
): QualitySummary {
  const rawByRole = new Map<string, unknown>();
  if (dataResults) {
    for (const { role, result } of dataResults) {
      if (result.success && result.data) rawByRole.set(role, result.data);
    }
  }
 ...
 */
export function checkDebateSummaryClean(
  summary: string,
  side: "bull" | "bear",
): string | null {
  ...
  const signatures: Array<{ pattern: RegExp; label: string }> = [
 ...
 */
export function checkResearchManagerConsistency(
  reasoning: string,
  bullScore: number,
  bearScore: number,
): string | null {
