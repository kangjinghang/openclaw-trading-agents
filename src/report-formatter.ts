...
import { QuickAnalysisResult, FullAnalysisResult } from "./types";
...
function directionEmoji(d: string): string {
  ...
  if (["看空", "sell", "underweight", "reject"].includes(lower)) return "🔴";
  return "🟡";
}
...
export function toMarkdown(result: AnyResult): string {
  const lines: string[] = [];
  const isFull = result.mode === "full";
  const full = isFull ? result as FullAnalysisResult : null;
...
export function toHtml(result: AnyResult): string {
  const isFull = result.mode === "full";
  const full = isFull ? result as FullAnalysisResult : null;
