 ...
 */
export function parseQualityReview(content: string): QualityReview | null {
  ...
  if (!CREDIBILITY_LEVELS.has(credibility)) return null;

  const coerceStrArray = (v: unknown): string[] =>
    Array.isArray(v) ? v.filter((x): x is string => typeof x === "string") : [];

  return {
    credibility: credibility as QualityReview["credibility"],
 ...
 */
export function formatQualityReview(review: QualityReview): string {
  const lines: string[] = ["\n## 数据可信度复核（LLM Layer-2）\n"];
...
export async function runQualityReview(
  ...
  const userMessage = loadAndRender(
    "quality_review.md",
    { ticker, date, grades_table: gradesTable, reports: reportsText },
    promptsBaseDir
  );
