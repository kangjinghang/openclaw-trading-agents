...
import { renderHotMoneySummary, renderQuarterlyTrends, renderConsensus, renderLockup, renderMacd, renderPe, formatAnalystPrompt, formatRiskPrompt } from "./shallow-analyzer";
...
function fieldRowHtml(label: string, htmlContent: string): string {
...
function summaryTableHtml(title: string, rows: string[][]): string {
  if (rows.length === 0) return "";
  let h = `<div class="summary-block"><h4>${title}</h4><table>`;
  for (const [k, v] of rows) h += `<tr><td class="lbl">${esc(k)}</td><td>${v}</td></tr>`;
  h += "</table></div>";
  return h;
}
...
function codeBlockStyled(content: string, style: "prompt" | "response" | ""): string {
...
function signedVal(v: number, decimals = 2, suffix = ""): string {
  const sign = v > 0 ? "+" : "";
...
function pctBar(pct: number): string {
  if (pct == null || !Number.isFinite(pct)) return `<span class="field-missing">N/A</span>`;
  const p = Math.max(0, Math.min(100, pct));
  let color: string;
  if (p < 30) color = "var(--ok)";
  else if (p < 70) color = "var(--warn)";
  else color = "var(--fail)";
...
/** 渲染信号标签（方向/状态） */
function signalTag(value: string, cls: string): string {
  if (!value) return `<span class="field-missing">无</span>`;
  return `<span class="signal ${cls}">${esc(value)}</span>`;
}

/** 根据 northbound_signal 返回标签 */
function northboundTag(signal: string): string {
  if (signal === "inflow") return signalTag("净流入", "inflow");
  if (signal === "outflow") return signalTag("净流出", "outflow");
  return `<span class="muted">${esc(signal || "无数据")}</span>`;
}

/** 根据 sector_in_industry_tag 返回标签 */
function sectorTag(tag: string): string {
  if (!tag) return `<span class="field-missing">未上榜</span>`;
  if (tag === "主线") return signalTag("主线", "bullish");
  if (tag === "弱势") return signalTag("弱势", "bearish");
  return signalTag(tag, "neutral");
}

/** 根据 fitness_score 返回色彩 class */
function fitnessClass(score: number): string {
  if (score >= 8) return "val-pos";
  if (score >= 5) return "val-warn";
  return "val-neg";
}

/** 根据 overall_risk 返回信号标签 */
function riskTag(risk: string): string {
  if (risk === "low") return signalTag("low", "bullish");
  if (risk === "medium") return signalTag("medium", "neutral");
  if (risk === "high") return signalTag("high", "bearish");
  return `<span class="muted">${esc(risk)}</span>`;
}

/** 根据 MACD crossover 返回信号标签 */
function crossoverTag(crossover: string): string {
  if (crossover === "golden") return signalTag("金叉", "bullish");
  if (crossover === "death") return signalTag("死叉", "bearish");
  return signalTag(crossover || "none", "neutral");
}

/** 根据 MACD direction 返回信号标签 */
function directionTag(direction: string): string {
  if (direction === "看多") return signalTag("看多", "bullish");
  if (direction === "看空") return signalTag("看空", "bearish");
...
function traceNews(d: StockData, idPrefix = ""): string {
  ...
  for (const n of d.news.slice(0, 5)) {
    ...
    if (n.source) h += ` <span class="muted">[${esc(n.source)}]</span>`;
    if (n.content) {
      // 过滤纯数字/表格噪音，只保留可读文本段
...
function traceHotMoney(d: StockData, idPrefix = ""): string {
  ...
  h += `<h4>注入 prompt</h4>`;
  h += codeBlockStyled(renderHotMoneySummary(d.hot_money), "prompt");
  return h;
}
...
function traceFundamentals(d: StockData, idPrefix = ""): string {
  ...
  if (d.fundamentals.quarterly_trends && d.fundamentals.quarterly_trends.length > 0) {
    h += `<h4>季度趋势</h4>`;
    h += codeBlockStyled(renderQuarterlyTrends(d.fundamentals.quarterly_trends), "prompt");
  }
  if (d.fundamentals.consensus_eps) {
    h += `<h4>机构预期</h4>`;
    h += codeBlockStyled(renderConsensus(d.fundamentals.consensus_eps), "prompt");
  }
  h += `<h4>注入 prompt</h4>`;
  ...
  const pe = renderPe(d.fundamentals.pe);  // 与实际注入 prompt 一致（亏损股归一化为 N/A）
...
function traceVpaMacd(d: StockData, idPrefix = ""): string {
  ...
  if (d.macd) {
    ...
    h += codeBlockStyled(renderMacd(d.macd), "prompt");
  }
  return h;
}
...
function traceLockup(d: StockData, idPrefix = ""): string {
  ...
  if (d.lockup) {
    ...
    h += `<h4>注入 prompt</h4>`;
    h += codeBlockStyled(renderLockup(d.lockup), "prompt");
  } else {
