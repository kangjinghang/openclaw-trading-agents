...
function fieldRow(label: string, value: unknown, unit = ""): string {
  if (value === undefined || value === null || value === "") return `| ${label} | _(缺失)_ |`;
...
function traceNews(d: StockData): string[] {
  ...
  for (const n of d.news.slice(0, 3)) {
    lines.push(`- **${n.title}**`);
    if (n.content) lines.push(`  正文摘要: ${n.content}`);
    if (n.time) lines.push(`  时间: ${n.time}`);
  }
  if (d.news.length > 3) lines.push(`- ... 共 ${d.news.length} 条`);
  lines.push("");
  lines.push("**→ 注入 prompt 的行**:");
  lines.push("```");
...
function traceHotMoney(d: StockData): string[] {
  ...
  lines.push("**→ 注入 prompt 的行**:");
  lines.push("```");
  lines.push(renderHotMoneySummary(d.hot_money));
  lines.push("```");
  return lines;
}
...
function traceFundamentals(d: StockData): string[] {
  ...
  if (d.fundamentals.quarterly_trends && d.fundamentals.quarterly_trends.length > 0) {
    ...
    lines.push("```");
    lines.push(renderQuarterlyTrends(d.fundamentals.quarterly_trends));
    lines.push("```");
  }
  if (d.fundamentals.consensus_eps) {
    lines.push("**机构预期（renderConsensus）**:");
    lines.push("```");
    lines.push(renderConsensus(d.fundamentals.consensus_eps));
    lines.push("```");
  }
  lines.push("");
  lines.push("**→ 注入 prompt 的行**:");
...
export function generateDataTraceReport(
  ...
  if (stockData.calls && stockData.calls.length > 0) {
    ...
    for (const c of stockData.calls) {
      ...
      if (c.response_snippet) rows.push(["响应内容", `\`\`\`json\n${c.response_snippet}\n\`\`\``]);
  ...
  lines.push("");
  lines.push("```markdown");
  lines.push(formatAnalystPrompt(stockData));
  lines.push("```");
