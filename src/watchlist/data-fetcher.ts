...
export function parseNews(raw: any): NewsItem[] {
  ...
  const items = list.map((n: any): NewsItem | null => {
  ...
  items.sort((a: NewsItem, b: NewsItem) => (b.time ?? "").localeCompare(a.time ?? ""));
  return items;
}
 ...
 *  全程容忍字段缺失，不抛异常。 */
export function parseHotMoney(raw: any, industry?: string): HotMoneyData {
  ...
  let dragonTigerRecent: string | undefined;
  let dragonTigerReason: string | undefined;
  const dt = Array.isArray(raw.dragon_tiger) ? raw.dragon_tiger : [];
  if (dt.length > 0) {
    const lines = dt.slice(0, 2).map((r: any): string => {
    ...
    if (reason0) dragonTigerReason = reason0.slice(0, 20);
  }

  // 板块轮动：inflow_top/outflow_top 取前 3 行业名，并判标的行业归属
  let sectorInflowTop: string | undefined;
  let sectorOutflowTop: string | undefined;
  let sectorTag = "";
  ...
  if (sff && typeof sff === "object") {
    ...
    if (industry) {
      const inHit = inflow.some((b: any) => typeof b?.name === "string" && b.name.includes(industry));
      const outHit = outflow.some((b: any) => typeof b?.name === "string" && b.name.includes(industry));
      sectorTag = inHit ? "主线" : outHit ? "弱势" : "未上榜";
    }
  }

  // 今日热门股：取前 3 条 name(reason)
  let hotStocksTop: string | undefined;
  const hs = Array.isArray(raw.hot_stocks) ? raw.hot_stocks : [];
  if (hs.length > 0) {
    hotStocksTop = hs.slice(0, 3).map((r: any): string => {
...
export interface MacroView {
  ...
  commodities?: Record<string, { label: string; chg_5d?: number; chg_20d?: number; trend?: string }>;
...
export function parseMacroView(raw: any): MacroView | null {
  if (!raw || typeof raw !== "object") return null;
  const sv = raw.sector_view;
  const view: MacroView = {};
  if (sv && typeof sv === "object") {
    if (typeof sv.market_view === "string" && sv.market_view) view.market_view = sv.market_view;
    if (typeof sv.pmi_signal === "string" && sv.pmi_signal) view.pmi_signal = sv.pmi_signal;
    if (Array.isArray(sv.bullish_sectors)) view.bullish_sectors = sv.bullish_sectors.filter((s: any) => typeof s === "string");
    if (Array.isArray(sv.bearish_sectors)) view.bearish_sectors = sv.bearish_sectors.filter((s: any) => typeof s === "string");
    if (sv.sector_scores && typeof sv.sector_scores === "object" && !Array.isArray(sv.sector_scores)) {
      view.sector_scores = sv.sector_scores as Record<string, number>;
    }
    if (Array.isArray(sv.indicators_used)) view.indicators_used = sv.indicators_used.filter((s: any) => typeof s === "string");
  }
  if (raw.commodities && typeof raw.commodities === "object" && !Array.isArray(raw.commodities)) {
    ...
    for (const [sym, val] of Object.entries(raw.commodities)) {
      if (val && typeof val === "object") {
        const c = val as any;
        const item: { label: string; chg_5d?: number; chg_20d?: number; trend?: string } = {
          label: typeof c.label === "string" ? c.label : sym,
        };
        if (typeof c.chg_5d === "number") item.chg_5d = c.chg_5d;
        if (typeof c.chg_20d === "number") item.chg_20d = c.chg_20d;
        if (typeof c.trend === "string" && c.trend) item.trend = c.trend;
        (view.commodities as Record<string, any>)[sym] = item;
      }
    }
  }
...
const MACRO_FETCH_TIMEOUT_MS = 90_000;

export async function fetchMacroData(date: string): Promise<MacroView | null> {
  try {
    const result = await execSkillScript("trading-news", "news", PROJECT_ROOT, ["--macro-only", "--date", date], null, MACRO_FETCH_TIMEOUT_MS);
...
export async function fetchStockData(
  ...
  sector: string,
  rankerThesis?: string,
  options?: { globalHotMoneyJson?: string; date?: string },
  ...
  if (options?.globalHotMoneyJson) hotMoneyArgs.push("--global-data", options.globalHotMoneyJson);
  const tasks = [
    ...
    safeCall(() => execSkillScript("trading-hot-money", "hot_money", PROJECT_ROOT, hotMoneyArgs)),
...
export async function fetchGlobalHotMoneyData(date: string): Promise<{
  ...
  try {
    const result = await execSkillScript(
      "trading-hot-money", "hot_money", PROJECT_ROOT,
      ["--ticker", "_global", "--date", date, "--global-only"],
    );
    if (!result?.success) return { globalHotMoney: null, calls: [] };
    return {
      globalHotMoney: {
