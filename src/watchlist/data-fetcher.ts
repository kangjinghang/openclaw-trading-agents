...
export function parseHotMoney(raw: any, industry?: string): HotMoneyData {
  ...
  if (sff && typeof sff === "object") {
    ...
    if (industry) {
      ...
      const norm = (s: string): string => s.replace(/[ⅠⅡⅢⅣⅤ二三四五]+$/, "").trim();
      const indNorm = norm(industry);
      // 双向包含：应对"白酒"vs"白酒Ⅱ"等命名粒度差异（任意一方含对方即命中）
      const match = (b: any): boolean => {
...
export function parseFundamentals(raw: any): { pe: number; pb: number; rev_q1: number; np_q1: number; industry: string; quarterly_trends?: QuarterlyTrend[]; consensus_eps?: ConsensusEps; pe_percentile?: number; pb_percentile?: number; capability_scores?: Record<string, { score: number; label: string }> } {
  ...
  return {
    ...
    rev_q1: (() => {
      const qt = Array.isArray(raw?.quarterly_trends) ? raw.quarterly_trends as Record<string, unknown>[] : [];
...
export async function fetchStockData(
  ...
  sector: string,
  rankerThesis?: string,
  options?: { globalHotMoney?: any; date?: string },
  ...
  const tasks = [
    ...
    safeCall(() => execSkillScript("trading-news", "news", PROJECT_ROOT, ["--ticker", ticker, "--date", today, "--lookback-days", "60", "--skip-macro", "--company-name", name])),
