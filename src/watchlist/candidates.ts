import type { DiffFile, RawSnapshotFile, CandidatesFile, CandidateEntry, RawRange, TopTrend } from "./types";
 ...
 * 上涨也可能下跌，取决于它最强的那条区间。
 */
function pickTopTrend(ranges: RawRange[] | undefined, scanDateMs: number): TopTrend | null {
...
function sortGroup(group: CandidateEntry[]): CandidateEntry[] {
  return [...group].sort((a, b) => {
    ...
    if (!ta && !tb) return 0;
    if (!ta) return 1;   // 无趋势排最后（理论上 neutral 已分离，防御）
    if (!tb) return -1;
    return (
      (tb.ongoing ? 1 : 0) - (ta.ongoing ? 1 : 0) ||
      tb.days - ta.days ||
...
export function buildCandidates(diff: DiffFile, rawToday: RawSnapshotFile): CandidatesFile {
  ...
  const down: CandidateEntry[] = [];
  const neutral: CandidateEntry[] = [];
