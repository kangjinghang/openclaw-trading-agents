...
export function baseWeight(fitness: number): number {
  if (fitness >= 9) return 0.07;
  if (fitness >= 8) return 0.05 + (fitness - 8) * 0.02; // 8→5%, 8.5→6%, 8.99→6.98%
  if (fitness >= 7) return 0.03 + (fitness - 7) * 0.02; // 7→3%, 7.5→4%
  return 0; // ≤6 不买
}
...
export function riskFactor(overallRisk: "low" | "medium" | "high"): number {
  switch (overallRisk) {
    case "low": return 1.0;
