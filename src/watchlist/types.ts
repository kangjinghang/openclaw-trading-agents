...
export interface CandidateEntry {
  ticker: string;
  name: string;
  range: RawRange;                   // 完整 range 字段(begin/end/type/percent/summary/points/url/title)
  range_kind: "continued" | "new";   // diff 里是 B1(continued) 还是 B2(new)
  days: number;                       // range 跨度天数(begin → end)
  range_events: RawReason[];          // 区间内单日异动事件链（涨跌都留）
}
...
export interface RankedEntry {
  ...
  percent: number;                   // 反查补齐
  days: number;                      // 反查补齐
  range_kind: "continued" | "new";   // 反查补齐
  reason: string;                    // LLM 给的评价
}
...
export interface ScanGroupBreakdown {
  range_kind: { continued: number; new: number };
  today_catalyst: {
    limit_up: number;
    pct_over_5: number;
    pct_under_5: number;
...
export interface ScanGroupResult {
  ...
  group: "LONG" | "SHORT";
  fallback: boolean;
  total_pre_filter?: number;         // SHORT only（共同过滤前 138）
  total_post_common_filter?: number; // SHORT only（共同过滤后 110）
  total: number;                     // 送入 LLM 的实际总数（LONG 35 / SHORT 44）
  distribution?: {                   // 决策时的分布快照（pool 非空时填）
    percent: DistributionStats;
    days: DistributionStats;
  };
  breakdown?: ScanGroupBreakdown;    // 决策时的分类计数（pool 非空时填）
  ranked_count: number;
...
export interface ScanSummary {
  scan_date: string;
  total_candidates: number;          // ranker 接到的 candidates 总数
  groups: {
    LONG: { total: number; ranked: number; excluded: number; fallback: boolean };
    SHORT: {
      total: number;
      pre_filter: number;
  ...
  top_picks: Array<RankedEntry & { group: "LONG" | "SHORT" }>;
}
