...
function bucket(fitness: number): string {
  if (fitness >= 9) return "9-10";
  if (fitness >= 8) return "8-8.9";
  if (fitness >= 7) return "7-7.9";
  if (fitness >= 6) return "6-6.9";
  return "≤5";
}

const BUCKET_ORDER = ["9-10", "8-8.9", "7-7.9", "6-6.9", "≤5"];

function stats(records: FitnessRecord[], minSamples: number): Record<string, { count: number; avg30d: number | null; avg14d: number | null; avg7d: number | null }> {
  const out: Record<string, { count: number; avg30d: number | null; avg14d: number | null; avg7d: number | null }> = {};
  for (const b of BUCKET_ORDER) out[b] = { count: 0, avg30d: null, avg14d: null, avg7d: null };

  const byBucket: Record<string, FitnessRecord[]> = {};
  ...
  for (const b of BUCKET_ORDER) {
    ...
    if (recs.length < minSamples) continue;  // 样本不足，不计算均值
    const avg = (key: "return_30d" | "return_14d" | "return_7d"): number | null => {
      const vals = recs.map(r => r[key]).filter((v): v is number => typeof v === "number");
...
function main() {
  ...
  const store = new FitnessHistoryStore(watchlistDir);
  ...
  if (actionFilter) console.log(`  action 过滤: ${actionFilter}`);
