...
describe("formatRiskPrompt", () => {
  it("包含同 analyst 的数据 + analyst 给的 thesis", () => {
    const data: any = {
      ...
      news: [{ title: "n1" }], hot_money: { northbound_yi: 0, northbound_signal: "", sector_in_industry_tag: "" }, fundamentals: { pe: 50, pb: 5, rev_q1: 1, np_q1: 0.1 },
    };
  ...
  it("注入 K 线数据（pct/支撑压力/量比）—— 修复占位符 bug", () => {
    const data: any = {
      ...
      news: [{ title: "n1" }], hot_money: { northbound_yi: 0, northbound_signal: "", sector_in_industry_tag: "" }, fundamentals: { pe: 50, pb: 5, rev_q1: 1e9, np_q1: 1e8 },
    };
    const analyst: AnalystReport = {
  ...
  it("注入 VPA 量价预计算结论（含顶部背离信号）", () => {
    const data: any = {
      ticker: "SZ300319", name: "麦捷科技", sector: "电子",
      kline: { pct_5d: 12, pct_20d: 45, support: 25, resistance: 30, volume_ratio_5_20: 0.6 },
      news: [], hot_money: { northbound_yi: 0, northbound_signal: "", sector_in_industry_tag: "" }, fundamentals: { pe: 0, pb: 0, rev_q1: 0, np_q1: 0 },
  ...
  it("注入基本面数据（PE/净利）—— risk-role 独立看数据做风险判断", () => {
    const data: any = {
      ticker: "SZ300319", name: "麦捷科技", sector: "电子",
      kline: { pct_5d: 12, pct_20d: 45, support: 25, resistance: 30, volume_ratio_5_20: 0.6 },
      news: [], hot_money: { northbound_yi: 0, northbound_signal: "", sector_in_industry_tag: "" }, fundamentals: { pe: 85, pb: 5, rev_q1: 1e9, np_q1: 1e8 },
    };
    const analyst: AnalystReport = {
      thesis: "x", fitness_score: 8, data_freshness: "2026-06-21", key_signals: [], data_gaps: [],
    };
...
describe("analyzeAll", () => {
  it("对每只股跑 2 calls（analyst + risk），返回 StockReport 数组", async () => {
    ...
    const dataByTicker = new Map<string, StockData>([
      ["SZ300319", { ticker: "SZ300319", name: "麦捷科技", sector: "电子", kline: { pct_5d: 1, pct_20d: 2, support: 1, resistance: 2, volatility_20d: 0.02, volume_ratio_5_20: 1.0 }, news: [], hot_money: { northbound_yi: 0, northbound_signal: "", sector_in_industry_tag: "" }, fundamentals: { pe: 1, pb: 1, rev_q1: 1, np_q1: 1, industry: "电子" } }],
      ["SH600183", { ticker: "SH600183", name: "生益科技", sector: "PCB", kline: { pct_5d: 1, pct_20d: 2, support: 1, resistance: 2, volatility_20d: 0.02, volume_ratio_5_20: 1.0 }, news: [], hot_money: { northbound_yi: 0, northbound_signal: "", sector_in_industry_tag: "" }, fundamentals: { pe: 1, pb: 1, rev_q1: 1, np_q1: 1, industry: "PCB" } }],
    ]);
    const mockCaller: ShallowLlmCaller = async ({ role }) => {
      if (role === "analyst") {
  ...
  it("单股 LLM 失败 → 该股跳过", async () => {
    ...
    for (const m of metas) {
      dataByTicker.set(m.ticker, { ticker: m.ticker, name: m.name, sector: "x", kline: { pct_5d: 1, pct_20d: 1, support: 1, resistance: 2, volatility_20d: 0.02, volume_ratio_5_20: 1.0 }, news: [], hot_money: { northbound_yi: 0, northbound_signal: "", sector_in_industry_tag: "" }, fundamentals: { pe: 1, pb: 1, rev_q1: 1, np_q1: 1, industry: "x" } });
    }
