...
- `financial_snapshot`：最新财务快照（含 `roe`、`operating_cash_flow` 等字段）。注：资产负债率/毛利率不在本子源（实时 TDX 协议无此字段），请分别查 `financial_health.debt_ratio_pct` 与 `quarterly_trends.gross_margin`
